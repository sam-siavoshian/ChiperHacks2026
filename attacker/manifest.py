...
def strike_list() -> list[dict[str, Any]]:
    """RED's handed-over targets: the confirmed real vulns (class + endpoint +
    injection point), generated by the arena build step. Empty list if the file is
    missing — the attacker can still recon the old way, it just loses the head start.
    """
    try:
        with open(config.STRIKE_LIST_PATH, encoding="utf-8") as f:
            data = json.load(f)
